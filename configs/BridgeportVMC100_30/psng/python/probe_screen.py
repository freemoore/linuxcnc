...
#        self.vcp_reload()
    ...
    def on_tool_change(self, gtkbutton, data=None):
        ...
        if change:
            if toolprepnumber == 0:
                # tool unload (T0) - nothing to probe
            ...
            # Run the same probing routine the "Probe Tool Setter" button
            # uses. ocode() blocks (polls to INTERP_IDLE) until it's
            # genuinely done - safe to chain the rest right after.
            if self.ocode("o<psng_probe_tool_setter> call") == -1:
                self.command.abort()
                ...
                self.halcomp["toolchange-changed"] = True
                self._notify(
                    _("**** TOOL #{0:d} PROBE FAILED - toolchange aborted ****").format(toolprepnumber),
                    timeout=0,
                )
                return

            self._notify(
                _("Tool #{0:d}  {1}  probed OK").format(toolprepnumber, tooldescr),
                timeout=4,
            )
            self.halcomp["toolchange-changed"] = True
        else:
    ...
    def _notify(self, message, timeout=4):
        """Non-blocking notice. timeout=0 stays up until closed by hand -
        used only for a real fault, where you want eyes on it."""
        dialog = Gtk.MessageDialog(
            self.window,
            Gtk.DialogFlags.DESTROY_WITH_PARENT,
        ...
        dialog.show_all()
        if timeout > 0:
            GLib.timeout_add_seconds(timeout, dialog.destroy)
