...
FACTOR_UNIT = 1
FACTOR_RES = 10.0
FACTOR_ALL = FACTOR_UNIT * FACTOR_RES
# 
UPDATE_INTERVAL = 0.04	# this is how often the z external offset value is updated based on current x & y position 
...
class Compensation :
	...
	def loadMap(self) :
		# data coordinates and values
                # Dian edit: factor by 25.4 to be compatible with Enlish units
		self.data = np.loadtxt(self.filename, dtype=float, delimiter=" ", usecols=(0, 1, 2)) * FACTOR_UNIT * FACTOR_RES
		self.x_data = np.around(self.data[:,0],1)
		self.y_data = np.around(self.data[:,1],1)
		self.z_data = self.data[:,2] / (FACTOR_UNIT * FACTOR_RES) 
		...
		print ("	xMin = ", self.xMin, " mm")
		print ("	xMax = ", self.xMax, " mm")
		print ("	yMin = ", self.yMin, " mm")
		print ("	yMax = ", self.yMax, " mm")

		# target grid to interpolate to, 1 * FACTOR_RES grid per mm
		self.xSteps = (self.xMax-self.yMin)+2
		self.ySteps = (self.yMax-self.yMin)+2
	...
	def compensate(self) :
		# get our nearest integer position
                # Dian edit: Factor by 25.4 to be compatible with English unit
		self.xpos = int(round(self.h['x-pos'] * FACTOR_UNIT * FACTOR_RES))
		self.ypos = int(round(self.h['y-pos'] * FACTOR_UNIT * FACTOR_RES))
